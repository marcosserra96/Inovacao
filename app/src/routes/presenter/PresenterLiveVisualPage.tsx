...
type Props = { sessionId: string; onOpenScreen?: () => void }

type FlowState = 'lobby' | 'prepare' | 'question' | 'reveal' | 'ranking' | 'quiz_result' | 'semifinal_ready' | 'semifinal' | 'semifinal_result' | 'final_ready' | 'final' | 'champion'

function Icon({ name, className = 'h-5 w-5' }: { name: 'users' | 'pause' | 'play' | 'signal' | 'flag' | 'trophy' | 'screen'; className?: string }) {
  const c = { fill: 'none', stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const }
  const paths = {
    users: <><path {...c} d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle {...c} cx="9" cy="7" r="4"/><path {...c} d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></>,
...
function BrandHeader({ onOpenScreen }: { onOpenScreen?: () => void }) {
  return (
...
export function PresenterLiveVisualPage({ sessionId, onOpenScreen }: Props) {
  ...
  return <main className="relative h-[100dvh] overflow-hidden bg-[#020d23] text-white">
    ...
    <div className="relative mx-auto grid h-full max-w-[1800px] grid-rows-[auto_minmax(0,1fr)] px-[clamp(14px,2.2vw,40px)] py-[clamp(10px,1.7vh,22px)]">
      <BrandHeader onOpenScreen={onOpenScreen}/>
