-- Fonte de tempo única para telão, apresentador e participante.
create or replace function public.get_live_dynamic_clock(p_session_id uuid)
returns json
language plpgsql
security definer
...
begin
  ...
  return json_build_object(
    ...
    'remainingMs', coalesce(v_session.flow_remaining_ms, 0),
    'updatedAt', v_session.updated_at
  );
end;
$$;

grant execute on function public.get_live_dynamic_clock(uuid) to anon, authenticated;

-- A última resposta passa a encerrar a pergunta atomicamente.
...
begin
  if not exists (
    select 1
    from live_quiz_participant_secrets
  ...
  if exists (
    select 1 from live_quiz_answers
  ...
  if v_round.is_tiebreaker
     and not (p_participant_id = any(v_round.tiebreak_participant_ids)) then
  ...
  v_elapsed_ms := v_round.timer_accumulated_ms
    + extract(epoch from (v_now - v_round.timer_started_at)) * 1000;
  ...
  if v_session.end_when_all_answered then
    ...
    if v_expected > 0 and v_answered >= v_expected then
      ...
      -- Não depende mais de polling/tick de nenhum navegador.
      update live_quiz_sessions
      set flow_state = 'reveal',
          phase = 'result_revealed',
          flow_deadline_at = v_now + make_interval(secs => greatest(1, coalesce(reveal_seconds, 5))),
          updated_at = v_now
      where id = v_round.session_id
